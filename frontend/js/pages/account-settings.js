...
const fields = { name: document.getElementById('customer-name'), email: document.getElementById('customer-email'), phone: document.getElementById('customer-phone'), current: document.getElementById('customer-current-password'), next: document.getElementById('customer-new-password'), confirm: document.getElementById('customer-confirm-password'), consent: form?.querySelector('[name="confirm_password_change"]') };
...
form?.addEventListener('submit', event => { event.preventDefault(); clearState(); if (!validate()) return; loading(true); window.setTimeout(() => { loading(false); status.textContent = 'Đã lưu thay đổi tài khoản.'; status.hidden = false; }, 280); });
